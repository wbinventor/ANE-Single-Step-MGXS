...
This paper uses Monte Carlo to generate MGXS for deterministic methods. The single-step framework used to generate MGXS from Monte Carlo is discussed in~\autoref{subsec:single-level}. The two pin-wise spatial homogeniation schemes used to quantify approximation error due to inter-pin spatial self-shielding model are introduced in~\autoref{subsec:homogenize}.
...
% In this way, the null scheme fully abandons the multi-level approach used by most traditional approaches to generate MGXS.
